...
const JsonViewer = ({ data, defaultExpanded = false }) => {
  ...
  if (typeof data !== 'object' || data === null) {
    return <span className="text-emerald-400 font-data text-xs">{JSON.stringify(data)}</span>;
  }
  ...
  if (keys.length === 0) {
    return <span className="text-zinc-600 font-data text-xs">{isArray ? '[]' : '{}'}</span>;
  }
  ...
  return (
    <div className="font-data text-xs">
      <button 
        ...
        className="flex items-center gap-1 hover:text-rose-400 transition-colors text-zinc-400"
      >
        {expanded ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
        <span>{isArray ? 'Array' : 'Object'}</span>
        <span className="text-zinc-600 text-[10px] ml-1">({keys.length} items)</span>
      </button>
        ...
        <div className="pl-4 mt-1 border-l border-zinc-800/50 space-y-1">
          {keys.map(key => (
            <div key={key} className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
              <span className="text-rose-300 min-w-fit">
                {isArray ? `[${key}]` : `"${key}"`}:
              </span>
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-8 lg:px-16 xl:px-24 pt-8 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold text-white">Raw Context Data</h2>
            <p className="text-[10px] text-zinc-600">Pipeline data exactly as assembled</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-8 lg:px-16 xl:px-24 pb-16 space-y-4">
        ...
        {data.context_prompt && (
          <div className="bg-zinc-900/60 rounded-xl border border-zinc-800/50 overflow-hidden">
            <div className="px-4 py-3 bg-rose-500/5 border-b border-zinc-800/50 font-medium text-rose-300 text-xs uppercase tracking-wide">
              Prompt sent to AI
            </div>
            <div className="p-4 overflow-x-auto">
              <pre className="text-[11px] text-rose-200/70 font-data whitespace-pre-wrap leading-relaxed">
                {data.context_prompt}
              </pre>
            </div>
          </div>
